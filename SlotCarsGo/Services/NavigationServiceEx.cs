...
namespace SlotCarsGo.Services
{
    public class NavigationServiceEx
    {
        ...
        private async void Frame_Navigated(object sender, NavigationEventArgs e)
        {
            ...
            if (dataContext != null)
            {
                await dataContext.OnNavigatedToAsync(e.Parameter, e.NavigationMode).ConfigureAwait(false);
            }
        }

        private INavigable ResolveForPage(Page page) => (page?.DataContext as INavigable);
